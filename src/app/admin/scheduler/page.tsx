...
import { Play, Square, RefreshCw, Clock, Calendar, TrendingUp, AlertCircle, CheckCircle, XCircle, Database } from 'lucide-react'
...
export default function SchedulerAdminPage() {
  ...
  const handleBulkHistorical = async () => {
    if (!confirm('Isso irá carregar dados históricos dos últimos 20 anos para TODOS os ativos. Esta operação pode demorar muito tempo. Continuar?')) {
      return
    }
  ...
  const getStatusBadge = (status: 'started' | 'completed' | 'failed') => {
    switch (status) {
      case 'completed':
        return <Badge variant="success" className="bg-green-100 text-green-800"><CheckCircle className="h-3 w-3 mr-1" />Sucesso</Badge>
      case 'failed':
        return <Badge variant="destructive"><XCircle className="h-3 w-3 mr-1" />Falha</Badge>
      case 'started':
        return <Badge variant="secondary"><RefreshCw className="h-3 w-3 mr-1 animate-spin" />Executando</Badge>
      default:
  ...
  const getTriggerBadge = (trigger: 'users_online' | 'scheduled' | 'force') => {
    switch (trigger) {
      case 'users_online':
        return <Badge variant="outline" className="bg-blue-50 text-blue-700">👥 Usuários Online</Badge>
      case 'scheduled':
        return <Badge variant="outline" className="bg-purple-50 text-purple-700">⏰ Agendado</Badge>
      case 'force':
        return <Badge variant="outline" className="bg-orange-50 text-orange-700">🔧 Manual</Badge>
      default:
        return <Badge variant="outline">Desconhecido</Badge>
    }
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Status do Scheduler</CardTitle>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {status?.hourlyJobRunning && status?.dailyJobRunning ? 'Ativo' : 'Inativo'}
            </div>
            <p className="text-xs text-muted-foreground">
              Jobs: {status?.hourlyJobRunning ? '✅' : '❌'} Horário | {status?.dailyJobRunning ? '✅' : '❌'} Diário
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {stats?.last24h.successful || 0}/{stats?.last24h.total || 0} execuções
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-4">
            <Button
              ...
              disabled={actionLoading !== null || (status?.hourlyJobRunning && status?.dailyJobRunning)}
              variant={status?.hourlyJobRunning && status?.dailyJobRunning ? 'secondary' : 'default'}
            >
            ...
            <Button
              ...
              variant="secondary"
            >
              {actionLoading === 'force_update' && <RefreshCw className="h-4 w-4 mr-2 animate-spin" />}
              <RefreshCw className="h-4 w-4 mr-2" />
              Atualização Manual
            </Button>
            ...
            <Button
              ...
              variant="outline"
            >
              {actionLoading === 'bulk_historical' && <RefreshCw className="h-4 w-4 mr-2 animate-spin" />}
            ...
            <div className="bg-muted/50 p-4 rounded-lg space-y-2">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div className="font-bold text-green-600">{stats.last24h.successful}</div>
                </div>
                <div>
                  <span className="text-muted-foreground">Falhas:</span>
                  <div className="font-bold text-red-600">{stats.last24h.failed}</div>
                </div>
                <div>
                ...
                <div key={log.id} className="border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline">
                        {log.type === 'hourly' && <Clock className="h-3 w-3 mr-1" />}
                        {log.type === 'daily' && <Calendar className="h-3 w-3 mr-1" />}
                        {log.type === 'manual' && <RefreshCw className="h-3 w-3 mr-1" />}
                        {log.type}
                      </Badge>
                    </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    {log.duration && (
                      <div>
                        ...
                        <div className="font-medium">{Math.round(log.duration / 1000)}s</div>
                      </div>
                    )}
                    {log.recordsUpdated !== undefined && (
                      <div>
                        <span className="text-muted-foreground">Atualizados:</span>
                        <div className="font-medium text-green-600">{log.recordsUpdated}</div>
                      </div>
                    )}
                    {log.errors !== undefined && (
                      <div>
                        <span className="text-muted-foreground">Erros:</span>
                        <div className="font-medium text-red-600">{log.errors}</div>
                      </div>
                    )}
                    <div>
                  ...
                  {log.details?.error && (
                    <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded text-sm text-red-700">
                      <strong>Erro:</strong> {log.details.error}
                    </div>
                  )}
                </div>
              ))}
