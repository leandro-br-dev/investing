...
export async function GET(req: NextRequest) {
  try {
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + dateStep)) {
      ...
      for (const item of filteredItems) {
        ...
        if (historicalPrice) {
          const itemValue = Number(item.quantity) * Number(historicalPrice.close)
      ...
      if (totalValue > 0) {
        ...
        const profitLossPercent = totalCost > 0 ? ((totalValue / totalCost) - 1) * 100 : 0
    ...
    const summary = performanceData.length > 0 ? {
      currentValue: performanceData[performanceData.length - 1]?.totalValue || 0,
      totalCost: performanceData[performanceData.length - 1]?.totalCost || 0,
      totalReturn: performanceData[performanceData.length - 1]?.profitLoss || 0,
      totalReturnPercent: performanceData[performanceData.length - 1]?.profitLossPercent || 0,
