  ...
  try {
    ...
    if (!portfolio) {
      ...
      const userExists = await prisma.user.findUnique({
        where: { id: (session as unknown).user.id },
      })
      ...
      if (userSettings && userSettings.minPurchaseIntervalDays > 0) {
        ...
        if (lastPurchase) {
          ...
          if (daysSinceLastPurchase < userSettings.minPurchaseIntervalDays) {
            const remainingDays =
              userSettings.minPurchaseIntervalDays - daysSinceLastPurchase
            return NextResponse.json(
              {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (type === "buy") {
        if (existingPosition) {
          ...
          if (!assetExists) {
            throw new Error(
              `Asset ${ticker} not found in database. Please ensure the asset is properly registered.`
            )
          }
        ...
        if (!existingPosition) {
          throw new Error(
            `No position found for ${ticker}. Cannot sell what you don't own.`
          )
        }

        if (Number(existingPosition.quantity) < Number(quantity)) {
          throw new Error(
            `Insufficient quantity. You have ${existingPosition.quantity} but trying to sell ${quantity}.`
          )
        }
